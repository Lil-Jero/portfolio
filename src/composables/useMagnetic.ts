...
export function useMagnetic(target: MaybeRefOrGetter<HTMLElement | null>) {
  ...
  const unsubscribe = [x.on('change', writeOffset('x')), y.on('change', writeOffset('y'))]
  ...
  const attract = (event: PointerEvent) => {
    ...
    x.set(clampOffset((event.clientX - (bounds.left + bounds.width / 2)) * ATTRACTION_RATIO))
    y.set(clampOffset((event.clientY - (bounds.top + bounds.height / 2)) * ATTRACTION_RATIO))
  }
